...
var failureDiv = document.querySelector(".failureDiv");
var emptyDiv = document.querySelector(".emptyDiv");
...
function checkURL(input) {
  var pattern = /^(https?:\/\/)?(www\.)?(amzn\.to|amazon\.com)\/.*$/;
  if (input === "") {
  ...
  clearTimeout(timer);
  var currentVisible = document.querySelector(
    '.overlay div[style="display: block;"]'
  );
  currentVisible.addEventListener("click", function () {
...
setTimeout(function () {
  // math
  let visitorCount = 10;
  let onlineVisitorStats = 20;
  let shortenedLinkCount = 50;
  var number = document.querySelectorAll(".numbers");
  number[0].innerHTML = visitorCount;
  number[1].innerHTML = onlineVisitorStats;
  number[2].innerHTML = shortenedLinkCount;
}, 1000);
